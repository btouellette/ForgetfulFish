...
### Known gaps (to close before the affected cards are considered rules-complete)

Tracked in `docs/plans/rules-engine/README.md` open questions 35–41. Each item names the smallest change that closes it.

1. **Target restrictions are not modelled.** `CardDefinition` declares no target requirements beyond what `OnResolveRegistry` infers ("object" / "player"), and `isLegalTarget` in `commands/validate.ts` only checks that the object still exists with the same ZCC and is not phased out — not its zone, type, or controller. "Target artifact" (Vision Charm), "target creature" (Ray of Command), "target creature you control" (Dance of the Skywise), and "target spell" (Memory Lapse) all accept any object. Close by adding a `targets` declaration to `CardDefinition` (`{ kind: "object"; zone; filter: ResolveCardFilter; controller? }` / `{ kind: "player" }`) enforced on cast and re-checked at resolution (CR 608.2b), with `OnResolveRegistry` reading it instead of scanning leaves.
2. **Only basic land types exist.** `ObjectSubtype` has a single `basic_land_type` variant, `choose_mode.modeSource: basic_land_types` enumerates the five basics, and `ResolveCardFilter.landType` only matches that variant. Vision Charm's first choice should offer every land type (CR 205.3i — Desert, Gate, Lair, Locus, Urza's, …) and the filter must match non-basic subtypes; Mystic Sanctuary and any future "Island" counting must not be broken by this. Close by adding a `land_type` subtype variant plus a `land_types` mode source and extending the filter.
3. **Type-changing effects do not swap intrinsic mana abilities (CR 305.7).** A Layer 4 `become_basic_land_type` changes the subtype only; the land keeps its printed mana ability. Close when basic-land identity (subtype ⇒ mana ability) is derived rather than printed.
4. **Modes are chosen on resolution, not on cast (CR 601.2b).** Vision Charm (and every `modal` / `choose_mode` spec) prompts while resolving, so a target is chosen before its mode is known and the mode is hidden from the opponent until resolution. Close by moving `modal` selection into `CAST_SPELL` (store the choice in `StackItem` scratch, have `OnResolveRegistry` compute target requirements for the chosen branch only).
5. **Phasing is a flag, not a rules subsystem.** `phasedOut` objects are still returned by battlefield zone selectors, static-ability counts (for example "other Islands"), continuous-effect application, and SBAs; attached objects do not phase with their parent (CR 702.26e); nothing triggers on phasing; phased-out objects are not redacted from opposing player views; phase-in is keyed to the controller's untap step without handling control changes while phased out (CR 702.26g). Close by treating `phasedOut` as a zone-like predicate inside `selectors.ts` / static-ability evaluation first, then the rest as cards demand.
6. **New cards are not reachable in real rooms.** `apps/server/src/room-store/deck-preset.ts` still lists only the Phase 2 cards; Diminishing Returns, Vision Charm, Ray of Command, and Dance of the Skywise are engine-only until `PS1.4` adds the preset-coverage test and the cards are added to the preset.
7. **`ResolutionCursor.step.index` is vestigial.** Nested position lives in scratch (`onResolvePath`); the cursor index only mirrors the top-level step for inspection. Remove or replace it with the path when the stack item view is next revised.
8. **Composable model limits.** `ResolvePlayerSelector` has no `each_opponent`, `ResolveAmount` has no `min`/`max`/comparison, `ResolveCondition` has no numeric comparison, and `ResolveCardsSelector` cannot select from the stack. Extend on demand; do not add card-specific kinds.
